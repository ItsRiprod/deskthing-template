...
function withHooks(rl2, cb) {
  const store = createStore(rl2);
...
function useKeypress(userHandler) {
  ...
  useEffect((rl2) => {
    ...
    const handler = withUpdates((_input, event) => {
      if (ignore)
        return;
      void signal.current(event, rl2);
    });
    rl2.input.on("keypress", handler);
    return () => {
      ignore = true;
      rl2.input.removeListener("keypress", handler);
    };
  }, []);
}
...
var ScreenManager = class {
  ...
  constructor(rl2) {
    this.rl = rl2;
    this.cursorPos = rl2.getCursorPos();
  }
  write(content) {
...
function createPrompt(view) {
  ...
  const prompt = (config, context = {}) => {
    ...
    const screen = new ScreenManager(rl2);
    ...
    rl2.input.on("keypress", checkCursorPos);
    cleanups.add(() => rl2.input.removeListener("keypress", checkCursorPos));
    return withHooks(rl2, (cycle) => {
      const hooksCleanup = import_node_async_hooks3.AsyncResource.bind(() => effectScheduler.clearAll());
      rl2.on("close", hooksCleanup);
      cleanups.add(() => rl2.removeListener("close", hooksCleanup));
      cycle(() => {
        try {
    ...
    if (isEnterKey(key)) {
      if (selectedChoice) {
        ...
        if (isValid === true) {
          ...
          rl2.write(selectedChoice.name);
          setSearchTerm(selectedChoice.name);
        }
      } else {
        rl2.write(searchTerm);
      }
...
var esm_default11 = createPrompt((config, done) => {
  ...
  useKeypress((key, rl2) => {
    ...
    if (isEnterKey(key)) {
    ...
    } else if (isNumberKey(key) && !Number.isNaN(Number(rl2.line))) {
      const position = Number(rl2.line) - 1;
      const item = items[position];
      ...
      const searchTerm = rl2.line.toLowerCase();
...
async function replacePlaceholders(destDir, data) {
  const getAllFiles = async (dir) => {
    ...
    const files2 = await Promise.all(dirents.map((dirent) => {
      const res = import_path.default.resolve(dir, dirent.name);
  ...
  await Promise.all(files.map(async (filePath) => {
    if (!filePath.includes("manifest.json") && !filePath.endsWith(".png") && !filePath.endsWith(".jpg")) {
      ...
      const rendered = (0, import_ejs.render)(content, data, {});
      await (0, import_promises.writeFile)(filePath, rendered, "utf-8");
    }
  }));
...
function getPackagePath() {
  try {
    if (typeof import_meta !== "undefined" && import_meta.url) {
      ...
      const __dirname2 = (0, import_node_path.dirname)(__filename);
      return (0, import_node_path.join)(__dirname2, "..", "package.json");
    } else {
      return (0, import_node_path.join)(__dirname, "..", "package.json");
    }
  } catch (error) {
    Logger.debug(`Error determining package path: ${error}`);
    return (0, import_node_path.join)(process.cwd(), "package.json");
  }
}
var packageJson;
try {
  ...
  packageJson = JSON.parse((0, import_node_fs.readFileSync)(packagePath, "utf-8"));
} catch (error) {
...
var version = packageJson.version || defaultValues.version;
var compatible_client = packageJson.compatible_client || defaultValues.compatible_client;
var compatible_server = packageJson.compatible_server || defaultValues.compatible_server;

// src/view/appManifest.ts
async function gatherAppManifest(projectName, templateId) {
  const label = await esm_default5({
    message: "App Label (Display Name):",
    default: projectName,
    validate: (input) => input.trim().length > 0 || "Label is required",
    transformer: (input) => input.trim()
  });
  const tags = await esm_default2({
    message: "Select app tags:",
    choices: Object.values(TagTypes).map((tag) => ({ value: tag, label: tag }))
  });
  const requires = await esm_default5({
    message: "Required App IDs (comma-separated):",
    transformer: (input) => input,
    validate: (input) => input.trim().length >= 0
  }).then((input) => input ? input.split(",").map((str) => str.trim()) : []);
  const appVersion = await esm_default5({
    message: `App Version (${version}):`,
    default: version,
    validate: (input) => {
      if (!(0, import_valid.default)(input)) {
        return "Invalid semantic version";
      }
      return input.trim().length > 0 || "Version is required";
    }
  });
  const requiredServerVersion = await esm_default5({
 ...
 (e.g. >=0.10.2, <=0.10.4) or ENTER to use default (>=${compatible_server})`,
    default: ">=0.0.0",
    transformer: (input) => input || `>=${compatible_server}`,
    validate: (input) => {
      if (!(0, import_valid2.default)(input)) {
        return "Invalid server version range";
      }
      return true;
    }
  });
  const requiredClientVersion = await esm_default5({
 ...
 (e.g. >=0.10.2, <=0.10.4) or ENTER to use default (>=${compatible_client})`,
  ...
  const description = await esm_default5({
    message: "App Description:",
    default: "None",
    validate: (input) => input.trim().length > 0 || "Description is required"
  });
  const author = await esm_default5({
    message: "Author:",
    default: "Unknown"
  });
  const platforms = await esm_default2({
  ...
  const repository = await esm_default5({
    message: "Github Repository URL (optional):"
  });
  const homepage = await esm_default5({
    message: "Homepage URL (optional):",
    default: repository || ""
  });
  const updateUrl = await esm_default5({
...
async function startCreation(type) {
  try {
    ...
    const destDir = import_path2.default.join(process.cwd(), projectName);
...
async function copyTemplate(source, destination) {
  ...
  await (0, import_promises2.cp)(source, destination, {
    recursive: true,
    filter: (src) => {
      const relativePath = import_path2.default.relative(source, src);
      const shouldExclude = excludeDirs.some(
        (dir) => relativePath === dir || relativePath.startsWith(`${dir}${import_path2.default.sep}`)
      );
      if (shouldExclude) {
...
async function createManifest(destDir, manifestData) {
  const manifestPath = import_path2.default.join(destDir, "deskthing", "manifest.json");
  await (0, import_promises2.mkdir)(import_path2.default.join(destDir, "deskthing"), { recursive: true });
  await (0, import_promises2.writeFile)(manifestPath, JSON.stringify(manifestData, null, 2));
...
async function updateProject(options = { noOverwrite: false }) {
  try {
    const publicPath = import_path3.default.join("./public/");
    const deskthingPath = import_path3.default.join("./deskthing/");
    const oldManifestPath = import_path3.default.join("./public/manifest.json");
    const manifestPath = import_path3.default.join("./deskthing/manifest.json");
    try {
      const [oldManifestExists, newManifestExists] = await Promise.all([
        (0, import_promises3.stat)(oldManifestPath).then(() => true).catch(() => false),
        (0, import_promises3.stat)(manifestPath).then(() => true).catch(() => false)
      ]);
      if (oldManifestExists && !newManifestExists) {
        Logger.warning(
          "Old manifest.json found. Migrating to the new file format in /deskthing"
        );
        await (0, import_promises3.mkdir)(deskthingPath, { recursive: true });
        for (const dir of ["images", "icons"]) {
          const oldDir = import_path3.default.join(publicPath, dir);
          const newDir = import_path3.default.join(deskthingPath, dir);
          try {
            await (0, import_promises3.stat)(oldDir);
            await (0, import_promises3.rename)(oldDir, newDir);
          } catch (e) {
          }
        }
        await (0, import_promises3.rename)(oldManifestPath, manifestPath);
        ...
        await (0, import_promises3.unlink)(oldManifestPath);
      } else if (!oldManifestExists && newManifestExists) {
    ...
    const manifestFile = await (0, import_promises3.readFile)(manifestPath, "utf-8");
    ...
    const answer = await esm_default5({
      ...
      validate: (input) => {
        if (!(0, import_valid3.default)(input)) {
          return "Invalid version format";
        }
        return true;
      }
    });
    const confirmAnswer = await esm_default4({
    ...
    const updatedManifest = {
      ...
      isAudioSource: manifestObject.isAudioSource || void 0,
      isScreenSaver: manifestObject.isScreenSaver || void 0,
      isWebApp: manifestObject.isWebApp || void 0,
      requires: manifestObject.requires || [],
      label: manifestObject.label || "Template App",
      version: answer || manifestObject.version || version,
      version_code: manifestObject.version_code || void 0,
      compatible_server: manifestObject.compatible_server || void 0,
      compatible_client: manifestObject.compatible_client || void 0,
      ...
      requiredVersions: confirmAnswer ? {
        ...
        server: `>=${manifestObject.compatible_server || compatible_server}`,
        client: `>=${manifestObject.compatible_client || compatible_client}`
      },
    ...
    if (!options.noOverwrite) {
      ...
      await Promise.all(
        templateFiles.map(async (file) => {
          ...
          try {
            await (0, import_promises3.stat)(templatePath);
          } catch (error) {
            Logger.debug(
          ...
          try {
            const templateContent = await (0, import_promises3.readFile)(templatePath, "utf-8");
            const userFile = import_path3.default.join(process.cwd(), file);
            await (0, import_promises3.mkdir)(import_path3.default.dirname(userFile), { recursive: true });
            try {
              const existingContent = await (0, import_promises3.readFile)(userFile, "utf-8");
              if (existingContent !== templateContent) {
                await (0, import_promises3.cp)(userFile, `${userFile}.backup`);
              }
            } catch (error) {
            }
            if (file === "package.json") {
              const currentPackage = JSON.parse(
                await (0, import_promises3.readFile)(userFile, "utf-8")
              );
              ...
              await (0, import_promises3.writeFile)(userFile, templateContent);
            }
          } catch (error) {
...
async function updateImports(filePath) {
  ...
  let updatedContent = content.replace(
    ...
    (match, imports) => {
      ...
      const lines2 = [];
      if (types.length) {
        lines2.push(`import { ${types.join(", ")} } from '@deskthing/types'`);
      }
      if (hasDeskthing) {
        lines2.push(`import { DeskThing } from '@deskthing/server'`);
      }
      return lines2.join("\n");
    }
  );
  updatedContent = updatedContent.replace(
    ...
    (match, imports) => {
      ...
      if (hasDeskthing) {
        lines2.push(`import { DeskThing } from '@deskthing/client'`);
      }
      return lines2.join("\n");
    }
  );
  if (filePath.toLowerCase().endsWith("server/index.ts") || filePath.toLowerCase().endsWith("server\\index.ts")) {
  ...
  await (0, import_promises3.writeFile)(filePath, updatedContent, "utf-8");
}
var findTypeScriptFiles = async (dir) => {
  const files = [];
  const items = await (0, import_promises3.readdir)(dir);
  for (const item of items) {
    const fullPath = import_path3.default.join(dir, item);
    if (await (0, import_promises3.stat)(fullPath).then((stats) => stats.isDirectory())) {
...
var noOverwrite = args.includes("--no-overwrite");
var isSilent = args.includes("--silent");
  ...
  try {
      ...
      choices: [
        {
          ...
          description: "Template with example code Vite + React + TS + Tailwindcss",
          value: "full"
        },
        {
          name: "Create minimal project",
          description: "Template with minimal code Vite + React + TS + Tailwindcss",
          value: "base"
        },
        {
    ...
    switch (setupChoice) {
      case "upgrade":
        ...
        const confirmRes = await esm_default4({
