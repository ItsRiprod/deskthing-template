...
// scripts/src/setup.ts
var rl = import_readline.default.createInterface({ input: process.stdin, output: process.stdout });
var askQuestion = (query) => new Promise((resolve) => rl.question(query, resolve));
var sourceDir = import_path.default.join(__dirname, "../../template");
var packageJson = JSON.parse(import_fs_extra.default.readFileSync(import_path.default.join(__dirname, "../../package.json"), "utf-8"));
async function init() {
  try {
    welcomeMessage(source_default);
    const projectName = (await askQuestion("Enter App ID (lowercase, no spaces): ")).toLowerCase().trim();
    const destDir = import_path.default.join(process.cwd(), projectName);
...
Creating project "${projectName}" in "${destDir}"...`);
    ...
    console.log(source_default.green(`DeskThing template created successfully at: ${destDir}`));
    await installDependencies(destDir, source_default);
    finalInstructions(projectName, source_default);
  } catch (error) {
...
function welcomeMessage(chalk2) {
  console.log(chalk2.cyanBright.bold(`Welcome to DeskThing v${packageJson.version}`));
  console.log(chalk2.cyanBright("I just need to ask you a few questions and you can be on your way!\n"));
  console.log(chalk2.greenBright("Let's set up your new project!\n"));
}
function validateProjectName(projectName, destDir) {
  ...
  if (import_fs_extra.default.existsSync(destDir)) {
...
async function createManifest(destDir, manifestData) {
  const manifestPath = import_path.default.join(destDir, "public", "manifest.json");
  await import_fs_extra.default.outputJson(manifestPath, manifestData, { spaces: 2 });
  console.log("Manifest file created successfully.\n");
}
async function replacePlaceholders(destDir, data) {
  const files = await import_fs_extra.default.readdir(destDir, { withFileTypes: true });
  for (const file of files) {
    ...
    if (file.isFile() && !file.name.includes("manifest.json")) {
      ...
      const rendered = import_ejs.default.render(content, data);
      await import_fs_extra.default.writeFile(filePath, rendered);
    }
  }
  console.log("Template placeholders replaced successfully.\n");
}
async function installDependencies(destDir, chalk2) {
  console.log(chalk2.blue("Installing dependencies..."));
  const spinner = ora("Running npm install...").start();
  await (0, import_child_process.exec)("npm install", { cwd: destDir }, (error, stdout, stderr) => {
    spinner.stop();
    if (error) {
      console.error(chalk2.red("Failed to install dependencies:", stderr));
      return;
    }
...
function finalInstructions(projectName, chalk2) {
  console.log(
    chalk2.greenBright("Setup complete! Now run the following commands:")
  );
  console.log(chalk2.bgBlack.bold(`  cd ${projectName}`));
  console.log(chalk2.bgBlack.bold("  npm run dev"));
  console.log(chalk2.greenBright("\nEnjoy your new DeskThing app!"));
}
init();
